...
MODEL=json.loads((DATA/"road_attribute_ml_model.json").read_text(encoding="utf-8"))
...
def main()->None:
    ...
    traffic=["registry_aadt","registry_pcu","registry_speed_kmh","traffic_source","traffic_value_status","traffic_model_id","traffic_model_confidence_pct","traffic_aadt_lower","traffic_aadt_upper","traffic_assignment_basis","road_name_display","road_name_assignment_basis","nearest_town","nearest_town_distance_km","nearest_reference_road","nearest_reference_road_distance_m","record_status"]
    ...
    connection.execute("INSERT INTO road_attribute_model_registry VALUES (?,?,?,?,?,?,?,?,?)",(MODEL["model_id"],MODEL["trained_at_utc"],"Extra Trees spatial-road ensemble",MODEL["targets"]["registry_aadt"]["observed_records"],MODEL["governed_links"]["records"],MODEL["hotosm_vehicular_network"]["features_enriched"],MODEL["targets"]["registry_aadt"]["validation_mae"],MODEL["targets"]["registry_aadt"]["validation_r2"],MODEL["estimation_policy"]))
