(function () {
  ...
  const LINK_FIELDS = [
    "link_id", "road_name", "start_x_coordinate_dd", "start_y_coordinate_dd", "end_x_coordinate_dd", "end_y_coordinate_dd", "x_coordinate_dd", "y_coordinate_dd", "coordinate_basis", "district", "county", "subcounty", "parish",
  ...
  const RECORD_FIELDS = {
    ...
    traffic: ["link_id", "road_name", "start_x_coordinate_dd", "start_y_coordinate_dd", "end_x_coordinate_dd", "end_y_coordinate_dd", "registry_aadt", "registry_pcu", "registry_speed_kmh", "geometry_length_km", "surface", "pavement_class", "condition", "district", "county", "subcounty", "parish", "planning_priority_score", "recommended_intervention"],
  ...
  function sparklineViz(values, unit) {
    const rows=vizValues(values), max=Math.max(...rows.map(item=>item.value),1), width=640, height=230, pad=48;
  ...
  function scatterViz(values, unit) {
    const rows=vizValues(values), max=Math.max(...rows.map(item=>item.value),1), maxCount=Math.max(...rows.map(item=>Number(item.count||1)),1), width=640, height=270, pad=48;
  ...
  function composedViz(values, unit) {
    const rows=vizValues(values), max=Math.max(...rows.map(item=>item.value),1), total=rows.reduce((sum,item)=>sum+item.value,0), width=640, height=270, pad=48, slot=(width-pad*2)/Math.max(rows.length,1);let running=0;
  ...
  function platformMindMapHtml(mindmap=cache.mindmap||{}, networkKm=Number(confirmedNetwork().length_km||0), mapMode=false) {
    const nodes=mindmap.nodes||[],edges=mindmap.edges||[];
    const nodeHtml=(node,kind)=>`<button class="mind-node ${kind}" type="button" title="${esc(node.label)} · ${number(edges.filter(edge=>edge.from===node.id||edge.to===node.id).length)} relationships"><strong>${esc(node.label)}</strong><small>${node.records?number(node.records)+" rows":number(edges.filter(edge=>edge.from===node.id||edge.to===node.id).length)+" relationships"}</small></button>`;
    return `<article class="matrix-card admin-block ${mapMode?"platform-map":""}"><header><div><small>Platform Architecture</small><h3>DUCAR Priority Studio Mind Map</h3></div><strong>${number(nodes.length)} nodes · ${number(edges.length)} relations</strong></header><div class="mind-map-canvas"><button class="mind-root" type="button">DUCAR Priority Studio<small>${number(networkKm,1)} km managed analytical network</small></button><div class="mind-branches"><section><h4>Reporting Sections</h4>${nodes.filter(node=>node.type==="section").map(node=>nodeHtml(node,"section")).join("")}</section><section><h4>Data And Evidence Stores</h4>${nodes.filter(node=>node.type==="data").map(node=>nodeHtml(node,"data")).join("")}</section></div></div></article>`;
  }
  ...
  function mapHtml() {
    ...
    const catalogues={
      ...
      traffic:[["hotosm","Traffic Analysis Extent · 248,616.15 km",true],["section","AADT Thematic Classification",true],["traffic","Traffic Observations Supplied",false],["traffic_missing","Traffic Observations Not Supplied",false],["aadt_low","AADT 0–149",false],["aadt_mid","AADT 150–499",false],["aadt_high","AADT 500–999",false],["aadt_very_high","AADT 1,000+",false],["pcu_supplied","PCU Supplied",false],["speed_low","Operating Speed Below 15 km/h",false]],
  ...
  function initSectionMap() {
    ...
    const labels=()=>L.tileLayer("https://{s}.basemaps.cartocdn.com/light_only_labels/{z}/{x}/{y}{r}.png",{maxZoom:19,attribution:"Labels © OpenStreetMap contributors · © CARTO",pane:"shadowPane"});
    const bases={hybrid:L.layerGroup([imagery(),labels()]),imagery:imagery(),dark:L.tileLayer("https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png",{maxZoom:19,attribution:"© OpenStreetMap · © CARTO"}),light:L.tileLayer("https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png",{maxZoom:19,attribution:"© OpenStreetMap · © CARTO"})};
    ...
    const configs={
      ...
      traffic:{filter:p=>typeof p.registry_aadt==="number",color:()=>"#bf5af2",weight:2.5},traffic_missing:{filter:p=>typeof p.registry_aadt!=="number",color:()=>"#65656d"},aadt_low:{filter:p=>Number.isFinite(p.registry_aadt)&&p.registry_aadt<150,color:()=>"#30d158"},aadt_mid:{filter:p=>p.registry_aadt>=150&&p.registry_aadt<500,color:()=>"#ffd60a"},aadt_high:{filter:p=>p.registry_aadt>=500&&p.registry_aadt<1000,color:()=>"#ff9f0a"},aadt_very_high:{filter:p=>p.registry_aadt>=1000,color:()=>"#ff375f",weight:3},pcu_supplied:{filter:p=>typeof p.registry_pcu==="number",color:()=>"#5e5ce6"},speed_low:{filter:p=>typeof p.registry_speed_kmh==="number"&&p.registry_speed_kmh<15,color:()=>"#ff375f"},
