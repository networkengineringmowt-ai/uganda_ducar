(function () {
  ...
  function initSectionMap() {
    ...
    const hotosmLayer=()=>L.geoJSON(cache.hotosmMap,{filter:feature=>headerMatches(feature.properties),renderer:L.canvas({padding:.6}),style:feature=>{const p=feature.properties,color=state.section==="traffic"?(p.registry_aadt>=1000?"#ff375f":p.registry_aadt>=500?"#ff9f0a":p.registry_aadt>=150?"#ffd60a":"#30d158"):"#00e5ff";return {color,weight:state.section==="traffic"?1.55:1.15,opacity:Math.min(roadOpacity(),state.section==="traffic"?.88:.7)};},onEachFeature:(feature,layer)=>layer.on("click",()=>selectFeature("HOTOSM road group",{...feature.properties,name:feature.properties.source_group_id},feature,layer))});
    ...
    const completeTrafficLayer=id=>L.geoJSON(cache.hotosmMap,{filter:feature=>headerMatches(feature.properties)&&configs[id].filter(feature.properties),renderer:L.canvas({padding:.6}),style:feature=>({color:configs[id].color(feature.properties),weight:configs[id].weight||2.7,opacity:roadOpacity()}),onEachFeature:(feature,layer)=>layer.on("click",()=>selectFeature("HOTOSM road group",{...feature.properties,name:feature.properties.source_group_id},feature,layer))});
