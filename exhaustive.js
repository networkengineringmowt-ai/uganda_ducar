(function () {
  ...
  const PATHS = {
    ...
    hotosmMap: "./data/hotosm_vehicular_map.geojson?v=20260824-thematic-audit-26",
    hotosmAnalysis: "./data/hotosm_vehicular_analysis.json",
    nationalMap: "./data/uganda_national_roads_2026.geojson?v=20260824-national-register-1"
  };
  ...
  function properRoadName(value) {
    ...
    return text.split(" - ").filter(Boolean).map(part=>part.split(" ").filter(Boolean).map(word=>acronyms.get(word.toLowerCase().replace(/\.$/,""))||word.charAt(0).toUpperCase()+word.slice(1).toLowerCase()).join(" ")).filter((part,index,array)=>array.findIndex(item=>item.toLowerCase()===part.toLowerCase())===index).join(" - ");
  }
  ...
  function recordsHtml() {
    ...
    const rows = filtered(dataset);
    const shownRows=rows.slice(0,Math.min(500,rows.length)),sectionTitle=SECTION_META[state.section]?.[0]||"DUCAR";
  ...
  function mountRemainingRecords(){
    ...
    const appendBatch=()=>{if(appending||token!==recordMountToken||!tbody.isConnected||index>=rows.length)return;appending=true;const end=Math.min(index+500,rows.length),html=rows.slice(index,end).map(row=>`<tr>${dataset.fields.map(field=>`<td class="${cellClass(field,row[field])}">${esc(displayCell(field,row[field]))}</td>`).join("")}</tr>`).join("");tbody.insertAdjacentHTML("beforeend",html);index=end;appending=false;updateStatus();if(rows.length<=10000&&index<rows.length)requestAnimationFrame(appendBatch);};
    wrap?.addEventListener("scroll",()=>{if(wrap.scrollTop+wrap.clientHeight>=wrap.scrollHeight-700)appendBatch();},{passive:true});updateStatus();if(rows.length<=10000)requestAnimationFrame(appendBatch);
  }
  ...
  function mapHtml() {
    const common=[["hotosm","Complete vehicular network · 248,616.15 km",false],["section","Complete section thematic roads",true],["paved","Paved roads · solid + thicker",false],["unpaved","Unpaved roads · dotted + thinner",false],["districts","District labels",false]],themes={
      ...
      socioeconomic:[["critical","High + Critical exposure",false]],ducar:[["national","Official national-road register · 21,136.75 km",false]],overview:[["national","Official national-road register · 21,136.75 km",false]],
  ...
  function initSectionMap() {
    ...
    const selectFeature=(kind,p,feature,layer)=>{if(!selectMode&&!measureMode)return;if(measureMode)return;if(selectedLayer)selectedLayer.remove();if(feature?.geometry?.type?.includes("Line"))selectedLayer=L.geoJSON(feature,{style:{color:"#fff",weight:3,opacity:1}}).addTo(map);else if(layer?.getLatLng)selectedLayer=L.circleMarker(layer.getLatLng(),{radius:10,color:"#fff",weight:3,fillColor:"#0a84ff",fillOpacity:.7}).addTo(map);details.innerHTML=reportHtml(kind,p);document.querySelector(".map-details")?.classList.add("has-selection");};
    const roadLayer=id=>L.geoJSON(cache.mapRoads,{filter:feature=>{const p=merged(feature);return headerMatches(p)&&configs[id].filter(p);},renderer:L.canvas({padding:.5}),style:feature=>{const p=merged(feature);return {color:configs[id].color(p),weight:roadWeight(p),dashArray:roadDash(p),lineCap:p.pavement_class==="Unpaved"?"round":"butt",opacity:roadOpacity()};},onEachFeature:(feature,layer)=>layer.on("click",()=>selectFeature("Road",merged(feature),feature,layer))});
    const completeLayerIds=new Set(["section","paved","unpaved","good","fair","poor","traffic","hightraffic"]), completeSections=new Set(["traffic","condition"]);
    const hotosmLayer=(id="hotosm")=>L.geoJSON(cache.hotosmMap,{filter:feature=>headerMatches(feature.properties)&&(!configs[id]||configs[id].filter(feature.properties)),renderer:L.canvas({padding:.6}),style:feature=>{const p=feature.properties,color=configs[id]?.color?.(p)||(state.section==="traffic"?(Number(p.registry_aadt)>=1000?"#ff1744":Number(p.registry_aadt)>=500?"#ff9100":Number(p.registry_aadt)>=150?"#7c4dff":"#00e5ff"):state.section==="condition"?({Good:"#00e676",Fair:"#ffd600",Poor:"#ff1744"})[p.condition]||"#9e9e9e":p.pavement_class==="Paved"?"#00e5ff":"#ffd600");return {color,weight:roadWeight(p),dashArray:roadDash(p),lineCap:p.pavement_class==="Unpaved"?"round":"butt",opacity:Math.min(roadOpacity(),.88)};},onEachFeature:(feature,layer)=>layer.on("click",()=>selectFeature("HOTOSM road group",{...feature.properties,name:feature.properties.source_group_id},feature,layer))});
    const nationalLayer=()=>L.geoJSON(cache.nationalMap,{renderer:L.canvas({padding:.6}),style:feature=>{const p=feature.properties;return {color:p.pavement_class==="Paved"?"#00ffff":"#ffe600",weight:p.pavement_class==="Paved"?2.3:1,dashArray:p.pavement_class==="Unpaved"?"2 5":null,opacity:.95};},onEachFeature:(feature,layer)=>layer.on("click",()=>selectFeature("Official national road",{...feature.properties,name:feature.properties.link_id},feature,layer))});
