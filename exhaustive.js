(function () {
  ...
  function scatterViz(values, unit) {
    ...
    return `<div class="advanced-viz"><svg class="scatter-chart" viewBox="0 0 ${width} ${height+34}" role="img" aria-label="Category value and frequency scatter"><line class="chart-axis" x1="${pad}" y1="${height-pad}" x2="${width-pad}" y2="${height-pad}"/><line class="chart-axis" x1="${pad}" y1="${pad}" x2="${pad}" y2="${height-pad}"/>${ticks}${rows.map((item,index)=>{const x=pad+(width-pad*2)*(index/Math.max(rows.length-1,1)),y=height-pad-(height-pad*2)*(item.value/max),r=5+13*Math.sqrt(Number(item.count||1)/maxCount);return `<circle cx="${x}" cy="${y}" r="${r}" fill="${categoryScaleColor(rows,index)}" fill-opacity=".78" stroke="#fff" stroke-opacity=".38"><title>${esc(properText(item.name))}: ${esc(chartNumber(item.value,unit))} · ${number(item.count||0)} records</title></circle>`;}).join("")}</svg>${vizLegend(rows,unit)}</div>`;
  }
  ...
  function rankedViz(values, unit) {
    const rows=vizValues(values), max=Math.max(...rows.map(item=>item.value),1);
    return `<div class="ranked-matrix">${rows.map((item,index)=>`<div><b>${index+1}</b><span title="${esc(item.name)}">${esc(item.name)}</span><i><em style="width:${item.value/max*100}%;background:${categoryScaleColor(rows,index)}"></em></i><strong>${esc(chartNumber(item.value,unit))}</strong><small>${number(item.count||0)} rec.</small></div>`).join("")}</div>`;
  }
  ...
  function lengthDashboard(rows) {
    ...
    if (state.section === "traffic") {const completeBand=key=>(cache.hotosmAnalysis?.traffic_2026?.[key]||[]).map(item=>({name:item.category,value:Number(item.length_km||0),length:Number(item.length_km||0),count:Number(item.feature_count||0)}));charts = [
      barChart("AADT Intensity","Complete road length and record frequency in every AADT band.",completeBand("aadt_band"),"affected km",COLORS[0]),
