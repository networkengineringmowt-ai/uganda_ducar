(function () {
  ...
  const PATHS = {
    ...
    hotosmMap: "./data/hotosm_vehicular_map.geojson.gz?v=20260828-corrected-network-1",
    hotosmDetailManifest: "./data/hotosm_detail_tiles_manifest.json?v=20260828-corrected-network-1",
    hotosmAnalysis: "./data/hotosm_vehicular_analysis.json?v=20260828-corrected-network-1",
    fullNetworkRecords: "./data/hotosm_vehicular_link_attributes.json.gz?v=20260828-corrected-network-1",
  ...
  function dashboardHtml() {
    ...
    const crossCut=`<section class="mandatory-breakdowns"><header><small>SECTION-OWNED COMPLETE BREAKDOWNS</small><h3>${esc(breakdown.title)}</h3><p>${esc(breakdown.subtitle)}</p></header><div class="chart-grid">${breakdown.series.map(([title,values])=>clusteredColumnChart(title,"Every available section record; affected length and record frequency shown together.",values,"affected km")).join("")}</div></section>${sectionGroupedClusterCharts()}`;
