(function () {
  ...
  const PATHS = {
    ...
    mapRoads: "./data/ducar_map_preview.geojson.gz?v=20260824-fast-map-4",
    structures: "./data/ducar_structure_analysis.json",
    structureMap: "./data/ducar_structures.geojson",
    hotosmMap: "./data/hotosm_vehicular_map.geojson.gz?v=20260824-source-alignment-1",
    hotosmDetailManifest: "./data/hotosm_detail_tiles_manifest.json?v=20260824-source-alignment-1",
    hotosmAnalysis: "./data/hotosm_vehicular_analysis.json?v=20260824-grouped-cross-analysis-1",
  ...
  function authoritativeNetworkOverview() {
    if(cache.hotosmAnalysis){const confirmed=confirmedNetwork(),total=cache.hotosmAnalysis.total||{};return `<section class="benchmark-panel authoritative-inventory"><header><div><small>HOTOSM VEHICULAR-ROAD INVENTORY - AUGUST 2026</small><h3>Uganda complete vehicular-road geometry</h3><p>The public length is calculated from every vehicular HOTOSM line feature in EPSG:32636. Pedestrian-only paths, footways, cycleways, steps, pedestrian ways, proposed ways and bridleways are excluded.</p></div><button class="pdf-download" data-section-pdf type="button">PDF report</button></header>${metricCards([{label:"Total vehicular-road length",value:number(confirmed.length_km,2)+" km",note:"Geometry-derived; no scaling"},{label:"Vehicular source features",value:number(total.feature_count),note:"Every eligible HOTOSM line"},{label:"Administrative districts",value:number(confirmed.districts),note:"Complete ADM2 coverage"},{label:"Surface-classified length",value:number(Number(total.paved_km||0)+Number(total.unpaved_km||0),2)+" km",note:"Complete observed + modelled planning classification"}])}<div class="chart-grid">${barChart("Road-management candidate classes","All vehicular geometry grouped by OSM functional class; candidates are analytical, not statutory ownership.",hotosmValues("management_class"),"affected km",COLORS[0])}${barChart("Pavement classification","Complete Paved and Unpaved planning classification; estimates retain explicit basis and confidence.",hotosmValues("pavement"),"affected km",COLORS[2])}${barChart("Condition classification","Complete condition planning classification; estimates retain explicit basis and confidence.",hotosmValues("condition"),"affected km",COLORS[1])}${barChart("Highway classification","All OSM vehicular highway classes remain intact; high-cardinality values are routed to tables.",hotosmValues("highway"),"affected km",COLORS[4])}</div><div class="method-note">Authoritative public total: ${number(confirmed.length_km,2)} km from ${number(total.feature_count)} vehicular source features. The web map preserves every eligible geometry in ${number(cache.hotosmMap?.metadata?.display_groups||2219)} non-selective display groups. Observed values are retained; estimated values carry status, method and confidence.</div></section>`;}
  ...
  function sectionClusteredBreakdowns() {
    ...
    const definitions={
      ducar:{title:"DUCAR Network Classification",subtitle:"Complete network management and pavement classifications.",series:[["Functional road-management class",hotosmValues("management_class")],["Pavement class",hotosmValues("pavement")]]},
      overview:{title:"DUCAR Network Classification",subtitle:"Complete network management and pavement classifications.",series:[["Functional road-management class",hotosmValues("management_class")],["Pavement class",hotosmValues("pavement")]]},
  ...
  function mapHtml() {
    ...
    const functionalMap=["ducar","overview"].includes(state.section),common=functionalMap?[["section","Complete network · functional classification",true],["districts","District labels",false],["nationalAligned","National-road spatial alignment within HOTOSM · 21,370.4 km",false],["national","Exact MoWT FY2025/26 national-road alignments & attributes",false]]:[["hotosm","Complete vehicular network · 248,616.15 km",false],["section","Complete section thematic roads",true],["nationalAligned","National-road spatial alignment within HOTOSM · 21,370.4 km",false],["national","Exact MoWT FY2025/26 national-road alignments & attributes",false],["paved","Paved roads · solid + thicker",false],["unpaved","Unpaved roads · dotted + thinner",false],["districts","District labels",false]],themes={
  ...
  function initSectionMap() {
    ...
    const functionalColor=p=>({"Community Access":RISK_SCALE[0],"Other Vehicular":RISK_SCALE[1],"Urban/Local":RISK_SCALE[2],"District/Collector":RISK_SCALE[3],"Other Trunk/Primary":RISK_SCALE[3],"Primary/National":RISK_SCALE[4]})[p.functional_class]||RISK_SCALE[2],trafficColor=p=>Number(p.registry_aadt)>=1500?RISK_SCALE[4]:Number(p.registry_aadt)>=1000?RISK_SCALE[3]:Number(p.registry_aadt)>=500?RISK_SCALE[2]:Number(p.registry_aadt)>=150?RISK_SCALE[1]:RISK_SCALE[0],sectionTheme=p=>["ducar","overview"].includes(state.section)?functionalColor(p):state.section==="socioeconomic"?({Low:RISK_SCALE[0],"Low-Moderate":RISK_SCALE[1],Moderate:RISK_SCALE[2],High:RISK_SCALE[3],Critical:RISK_SCALE[4]})[p.exposure_band]||RISK_SCALE[2]:state.section==="traffic"?trafficColor(p):mapColor(p);
    ...
    const setLayerStat=(id,stat)=>{if(!stat)return;if(id==="nationalAligned")return void(stat.textContent="21,370.4 km");if(id==="national")return void(stat.textContent=number(cache.nationalMap?.metadata?.registry_length_km||21136.748162,2)+" km");if(id==="hotosm")return void(stat.textContent=number(cache.hotosmMap?.metadata?.geometry_length_km||248616.148981,2)+" km");if(!configs[id])return;const source=useComplete(id)&&cache.hotosmMap?cache.hotosmMap.features.map(feature=>feature.properties):cache.mapRoads.features.map(merged),values=source.filter(p=>headerMatches(p)&&configs[id].filter(p)),km=values.reduce((sum,p)=>sum+Number(p.geometry_length_km||0),0);stat.textContent=number(km,1)+" km";};
