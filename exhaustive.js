(function () {
  ...
  const PATHS = {
    ...
    hotosmMap: "./data/hotosm_vehicular_map.geojson?v=20260824-route-conflation",
  ...
  function ensureHeaderExportMenu() {
    ...
    wrapper.addEventListener("click",event=>{const option=event.target.closest?.('[data-header-export="route-csv"],[data-header-export="lineage-csv"],[data-header-export="route-audit"]');if(!option)return;close();const files={"route-csv":["./data/hotosm_vehicular_route_register.csv.gz","uganda_hotosm_267903_merged_route_register.csv.gz"],"lineage-csv":["./data/hotosm_segment_route_relations.csv.gz","uganda_hotosm_segment_to_route_lineage.csv.gz"],"route-audit":["./data/hotosm_route_conflation_2026.json","uganda_hotosm_route_conflation_audit_2026.json"]},[href,download]=files[option.dataset.headerExport],anchor=document.createElement("a");anchor.href=href;anchor.download=download;anchor.click();});
  ...
  const navObserver=new MutationObserver(()=>{if(navHealthy()||navRepairTimer)return;navRepairTimer=setTimeout(()=>{navRepairTimer=null;ensureSocioeconomicNav();ensureStructuresNav();syncPrimaryNav();},60);});
