(function () {
  ...
  function initGlobalMap() {
    ...
    const rows=globalRows(),map=L.map(node,{preferCanvas:true,zoomControl:false,doubleClickZoom:false,worldCopyJump:true}).setView([18,8],2);
    ...
    bases.hybrid.addTo(map);transport.addTo(map);L.control.scale({imperial:false,maxWidth:160,position:"bottomleft"}).addTo(map);
    ...
    document.querySelectorAll("[data-map-tool]").forEach(button=>button.addEventListener("click",async()=>{const tool=button.dataset.mapTool;if(tool==="basemap")return;if(tool==="zoom-in")map.zoomIn();else if(tool==="zoom-out")map.zoomOut();else if(tool==="pan"){measureMode=false;}else if(tool==="select"){measureMode=false;}else if(tool==="measure"){measureMode=true;}else if(tool==="clear")clearMap();else if(tool==="reset")map.setView([18,8],2);else if(tool==="download")downloadElementPng(document.querySelector(".map-stage"),"ducar_global_road_inventory_map.png");else if(tool==="fullscreen"){const workspace=document.getElementById("map-workspace");if(workspace.requestFullscreen)await workspace.requestFullscreen();else workspace.classList.add("map-fullscreen-fallback");setTimeout(()=>map.invalidateSize(),120);}else if(tool==="restore"){if(document.fullscreenElement)await document.exitFullscreen();document.getElementById("map-workspace")?.classList.remove("map-fullscreen-fallback");setTimeout(()=>map.invalidateSize(),120);}document.querySelectorAll("[data-map-tool]").forEach(item=>item.classList.toggle("active",item===button&&["pan","select","measure"].includes(tool)));}));
