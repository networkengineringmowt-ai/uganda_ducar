(function () {
  ...
  function orderedCategoryScore(name){
    ...
    if(/critical|severe|very poor|\bpoor\b|failed|1,500\+|75\s*(?:to|–|-)\s*100/.test(value))return 1;
    if(/\bhigh\b|1,000\+|1,000\s*(?:to|–|-)\s*1,499/.test(value))return .75;
    if(/moderate|\bfair\b|500\s*(?:to|–|-)\s*999|50\s*(?:to|–|-)\s*74/.test(value))return .5;
    if(/low[- ]moderate|150\s*(?:to|–|-)\s*499|25\s*(?:to|–|-)\s*49/.test(value))return .25;
    if(/\blow\b|\bgood\b|below|0\s*(?:to|–|-)\s*(?:24|149|499)/.test(value))return 0;
    return null;
  }
  ...
  function sectionClusteredBreakdowns() {
    ...
    const definitions={
      ...
      traffic:{title:"Traffic Intelligence",subtitle:"Traffic volume and operating-speed bands for the complete traffic reporting population.",series:[["AADT volume bands",numeric(links,"registry_aadt",[["Below 150",0,150],["150–499",150,500],["500–999",500,1000],["1,000–1,499",1000,1500],["1,500+",1500,Infinity]])],["Operating-speed bands",numeric(links,"registry_speed_kmh",[["Below 20 km/h",0,20],["20–39 km/h",20,40],["40–59 km/h",40,60],["60–79 km/h",60,80],["80+ km/h",80,Infinity]])]]},
  ...
  function initGlobalMap() {
    ...
    const rows=globalRows(),worldExtent=L.latLngBounds([[-60,-180],[82,180]]),map=L.map(node,{preferCanvas:true,zoomControl:false,doubleClickZoom:false,worldCopyJump:true,minZoom:1});
    const imagery=L.tileLayer("https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}",{maxZoom:19,attribution:"Imagery © Esri"}),transport=L.tileLayer("https://server.arcgisonline.com/ArcGIS/rest/services/Reference/World_Transportation/MapServer/tile/{z}/{y}/{x}",{maxZoom:19,opacity:.88,attribution:"Road cartography © Esri contributors"}),boundaries=L.tileLayer("https://services.arcgisonline.com/ArcGIS/rest/services/Reference/World_Boundaries_and_Places/MapServer/tile/{z}/{y}/{x}",{maxZoom:19,attribution:"Boundaries and labels © Esri"});
  ...
  function mapHtml() {
    ...
    const key={ducar:[["#ff1744","Primary / National"],["#ff9100","District / Collector"],["#7c4dff","Urban / Local"],["#00e676","Community Access"],["#00e5ff","Other Vehicular"]],overview:[["#ff1744","Primary / National"],["#ff9100","District / Collector"],["#7c4dff","Urban / Local"],["#00e676","Community Access"],["#00e5ff","Other Vehicular"]],traffic:[["#22c55e","AADT below 150"],["#84cc16","AADT 150–499"],["#eab308","AADT 500–999"],["#f97316","AADT 1,000–1,499"],["#ef4444","AADT 1,500+"]],condition:[["#22c55e","Good"],["#eab308","Fair"],["#ef4444","Poor"]],structures:[["#00e5ff","Bridge"],["#ff2d95","Major Culvert"],["#ff9100","Other Structure"]],socioeconomic:[["#22c55e","Low Exposure"],["#eab308","Moderate Exposure"],["#f97316","High Exposure"],["#ef4444","Critical Exposure"]]}[state.section]||[["#22c55e","Low / Good"],["#84cc16","Low-Moderate"],["#eab308","Moderate / Fair"],["#f97316","High"],["#ef4444","Critical / Severe"]];
  ...
  function mapColor(properties) {
    ...
    if (["pims","budgets","hdm4"].includes(state.section)) return ({Low:RISK_SCALE[0],Moderate:RISK_SCALE[2],High:RISK_SCALE[3],Critical:RISK_SCALE[4]})[properties.priority_band] || "#8e8e93";
  ...
  function initSectionMap() {
    ...
    const imagery=L.tileLayer("https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}",{maxZoom:19,attribution:"Tiles © Esri"}),hybridReference=L.tileLayer("https://services.arcgisonline.com/ArcGIS/rest/services/Reference/World_Boundaries_and_Places/MapServer/tile/{z}/{y}/{x}",{maxZoom:19,attribution:"Boundaries and labels © Esri"});
    ...
    const functionalColor=p=>({"Primary/National":"#ff1744","District/Collector":"#ff9100","Urban/Local":"#7c4dff","Community Access":"#00e676","Other Vehicular":"#00e5ff"})[p.functional_class]||"#aeb7c4",trafficColor=p=>Number(p.registry_aadt)>=1500?RISK_SCALE[4]:Number(p.registry_aadt)>=1000?RISK_SCALE[3]:Number(p.registry_aadt)>=500?RISK_SCALE[2]:Number(p.registry_aadt)>=150?RISK_SCALE[1]:RISK_SCALE[0],sectionTheme=p=>["ducar","overview"].includes(state.section)?functionalColor(p):state.section==="socioeconomic"?({Low:RISK_SCALE[0],Moderate:RISK_SCALE[2],High:RISK_SCALE[3],Critical:RISK_SCALE[4]})[p.exposure_band]||"#9e9e9e":state.section==="traffic"?trafficColor(p):mapColor(p);
    const roadWeight=p=>p.pavement_class==="Paved"?1.8:.8, roadDash=p=>p.pavement_class==="Unpaved"?"2 5":null;
    const configs={
      section:{filter:()=>true,color:sectionTheme},all:{filter:()=>true,color:()=>"#00e5ff"},paved:{filter:p=>p.pavement_class==="Paved",color:()=>"#00e5ff"},unpaved:{filter:p=>p.pavement_class==="Unpaved",color:()=>"#ffd600"},good:{filter:p=>p.condition==="Good",color:()=>RISK_SCALE[0]},fair:{filter:p=>p.condition==="Fair",color:()=>RISK_SCALE[2]},poor:{filter:p=>p.condition==="Poor",color:()=>RISK_SCALE[4]},traffic:{filter:p=>typeof p.registry_aadt==="number",color:trafficColor},hightraffic:{filter:p=>Number(p.registry_aadt)>=1000,color:trafficColor},nationalAligned:{filter:p=>Boolean(p.national_aligned),color:()=>"#ffffff"},critical:{filter:p=>["Critical","High"].includes(p.priority_band||p.exposure_band),color:p=>(p.priority_band||p.exposure_band)==="Critical"?RISK_SCALE[4]:RISK_SCALE[3]},maintenance:{filter:p=>String(p.recommended_intervention||"").toLowerCase().includes("maintenance"),color:()=>RISK_SCALE[0]}
    };
    ...
    const nationalLayer=()=>L.geoJSON(cache.nationalMap,{renderer:L.svg({padding:.8}),smoothFactor:0,style:feature=>{const p=feature.properties;return {color:p.pavement_class==="Paved"?"#00ffff":"#ffe600",weight:p.pavement_class==="Paved"?2.4:1.2,dashArray:p.pavement_class==="Unpaved"?"2 5":null,lineCap:"round",lineJoin:"round",opacity:1};},onEachFeature:(feature,layer)=>{const p=feature.properties;layer.bindTooltip(selectionTip(p),{sticky:true,direction:"top",opacity:.96});layer.on("click",()=>selectFeature("Official national road",{...p,name:p.link_id},feature,layer));}});
    ...
    const detailColor=p=>["ducar","overview"].includes(state.section)?functionalColor(p):state.section==="traffic"?trafficColor(p):state.section==="condition"?({Good:RISK_SCALE[0],Fair:RISK_SCALE[2],Poor:RISK_SCALE[4]})[p.condition]||"#9e9e9e":["pims","hdm4","budgets"].includes(state.section)?({Low:RISK_SCALE[0],Moderate:RISK_SCALE[2],High:RISK_SCALE[3],Critical:RISK_SCALE[4]})[p.road_safety_risk_band]||"#8b5cf6":state.section==="structures"?"#bf5af2":state.section==="socioeconomic"?"#00e5ff":mapColor(p);
    ...
    map.on("moveend zoomend",()=>updateDetailTiles());
