(function () {
  ...
  function initSectionMap() {
    ...
    const useComplete=id=>id==="nationalAligned"||(completeSections.has(state.section)&&completeLayerIds.has(id)), createLayer=id=>id==="hotosm"||useComplete(id)?hotosmLayer(id):id==="national"?nationalLayer():id==="districts"?districtLayer():id.includes(":")?pointLayer(id):roadLayer(id);
    document.querySelectorAll("[data-map-layer]").forEach(input=>{const id=input.dataset.mapLayer,stat=document.querySelector(`[data-layer-stat="${id}"]`);if(configs[id]){const values=(useComplete(id)?cache.hotosmMap.features.map(feature=>feature.properties):cache.mapRoads.features.map(merged)).filter(p=>headerMatches(p)&&configs[id].filter(p)),km=values.reduce((s,p)=>s+Number(p.geometry_length_km||0),0);if(stat)stat.textContent=number(km,1)+" km";}else if(id==="hotosm"&&stat)stat.textContent=number(cache.hotosmMap?.metadata?.geometry_length_km||248616.148981,2)+" km";else if(id==="national"&&stat)stat.textContent=number(cache.nationalMap?.metadata?.registry_length_km||21136.748162,2)+" km";const toggle=async()=>{if(input.checked){if(id==="national"&&!cache.nationalMap){if(stat)stat.textContent="Loading…";try{await data("nationalMap");}catch(error){input.checked=false;if(stat)stat.textContent="Load failed";details.innerHTML=`<div class="map-empty-state"><b>!</b><strong>National Layer Could Not Load</strong><span>${esc(error.message)}</span></div>`;return;}if(stat)stat.textContent=number(cache.nationalMap.metadata?.registry_length_km||21136.748162,2)+" km";if(!input.checked)return;}const layer=activeLayers.get(id)||createLayer(id);activeLayers.set(id,layer);layer.addTo(map);}else activeLayers.get(id)?.remove();};input.addEventListener("change",toggle);if(input.checked)toggle();});
