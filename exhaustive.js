(function () {
  ...
  async function ensureData() {
    ...
    if (state.tab === "map") {
      ...
      if (["ducar","overview","global"].includes(state.section)) await data("nationalMap");
  ...
  function mapHtml() {
    const functionalMap=["ducar","overview"].includes(state.section),common=functionalMap?[["section","Complete network · functional classification",true],["districts","District labels",false],["nationalAligned","HOTOSM roads aligned to the national register",false],["national","Actual MoWT national-road alignments & attributes",false]]:[["hotosm","Complete vehicular network · 248,616.15 km",false],["section","Complete section thematic roads",true],["paved","Paved roads · solid + thicker",false],["unpaved","Unpaved roads · dotted + thinner",false],["districts","District labels",false]],themes={
    ...
    const key={ducar:[["#ff1744","Primary / National"],["#ff9100","District / Collector"],["#7c4dff","Urban / Local"],["#00e676","Community Access"],["#00e5ff","Other Vehicular"]],overview:[["#ff1744","Primary / National"],["#ff9100","District / Collector"],["#7c4dff","Urban / Local"],["#00e676","Community Access"],["#00e5ff","Other Vehicular"]],traffic:[["#00e5ff","AADT below 150"],["#7c4dff","AADT 150–499"],["#ff9100","AADT 500–999"],["#ff1744","AADT 1,000+"]],condition:[["#00e676","Good"],["#ffd600","Fair"],["#ff1744","Poor"]],structures:[["#00e5ff","Bridge"],["#ff2d95","Major Culvert"],["#ff9100","Other Structure"]],socioeconomic:[["#00e676","Low Exposure"],["#ffd600","Moderate Exposure"],["#ff9100","High Exposure"],["#ff1744","Critical Exposure"]]}[state.section]||[["#00e676","Low / Good"],["#ffd600","Moderate / Fair"],["#ff9100","High"],["#ff1744","Critical / Poor"]];
  ...
  function initSectionMap() {
    ...
    const functionalColor=p=>({"Primary/National":"#ff1744","District/Collector":"#ff9100","Urban/Local":"#7c4dff","Community Access":"#00e676","Other Vehicular":"#00e5ff"})[p.functional_class]||"#aeb7c4",sectionTheme=p=>["ducar","overview"].includes(state.section)?functionalColor(p):state.section==="socioeconomic"?({Low:"#00e676",Moderate:"#ffd600",High:"#ff9100",Critical:"#ff1744"})[p.exposure_band]||"#9e9e9e":state.section==="traffic"?(Number(p.registry_aadt)>=1000?"#ff1744":Number(p.registry_aadt)>=500?"#ff9100":Number(p.registry_aadt)>=150?"#7c4dff":"#00e5ff"):mapColor(p);
    const roadWeight=p=>p.pavement_class==="Paved"?1.8:.8, roadDash=p=>p.pavement_class==="Unpaved"?"2 5":null;
    const configs={
      section:{filter:()=>true,color:sectionTheme},all:{filter:()=>true,color:()=>"#00e5ff"},paved:{filter:p=>p.pavement_class==="Paved",color:()=>"#00e5ff"},unpaved:{filter:p=>p.pavement_class==="Unpaved",color:()=>"#ffd600"},good:{filter:p=>p.condition==="Good",color:()=>"#00e676"},fair:{filter:p=>p.condition==="Fair",color:()=>"#ffd600"},poor:{filter:p=>p.condition==="Poor",color:()=>"#ff1744"},traffic:{filter:p=>typeof p.registry_aadt==="number",color:sectionTheme},hightraffic:{filter:p=>Number(p.registry_aadt)>=1000,color:()=>"#ff1744"},nationalAligned:{filter:p=>Boolean(p.national_aligned),color:()=>"#ffffff"},critical:{filter:p=>["Critical","High"].includes(p.priority_band||p.exposure_band),color:p=>(p.priority_band||p.exposure_band)==="Critical"?"#ff1744":"#ff9100"},maintenance:{filter:p=>String(p.recommended_intervention||"").toLowerCase().includes("maintenance"),color:()=>"#00e676"}
    };
    ...
    const hotosmLayer=(id="hotosm")=>L.geoJSON(cache.hotosmMap,{filter:feature=>headerMatches(feature.properties)&&(!configs[id]||configs[id].filter(feature.properties)),renderer:L.canvas({padding:.6}),style:feature=>{const p=feature.properties,color=configs[id]?.color?.(p)||(state.section==="traffic"?(Number(p.registry_aadt)>=1000?"#ff1744":Number(p.registry_aadt)>=500?"#ff9100":Number(p.registry_aadt)>=150?"#7c4dff":"#00e5ff"):state.section==="condition"?({Good:"#00e676",Fair:"#ffd600",Poor:"#ff1744"})[p.condition]||"#9e9e9e":p.pavement_class==="Paved"?"#00e5ff":"#ffd600");return {color,weight:roadWeight(p),dashArray:roadDash(p),lineCap:p.pavement_class==="Unpaved"?"round":"butt",opacity:Math.min(roadOpacity(),.88)};},onEachFeature:(feature,layer)=>{const p=feature.properties;layer.bindTooltip(selectionTip(p),{sticky:true,direction:"top",opacity:.94});layer.on("click",()=>selectFeature("HOTOSM road group",{...p,name:p.source_group_id},feature,layer));}});
    const nationalLayer=()=>L.geoJSON(cache.nationalMap,{renderer:L.canvas({padding:.6}),style:feature=>{const p=feature.properties;return {color:p.pavement_class==="Paved"?"#00ffff":"#ffe600",weight:p.pavement_class==="Paved"?2.3:1,dashArray:p.pavement_class==="Unpaved"?"2 5":null,opacity:.95};},onEachFeature:(feature,layer)=>{const p=feature.properties;layer.bindTooltip(selectionTip(p),{sticky:true,direction:"top",opacity:.96});layer.on("click",()=>selectFeature("Official national road",{...p,name:p.link_id},feature,layer));}});
    ...
    const useComplete=id=>completeSections.has(state.section)&&completeLayerIds.has(id), createLayer=id=>id==="hotosm"||useComplete(id)?hotosmLayer(id):id==="national"?nationalLayer():id==="districts"?districtLayer():id.includes(":")?pointLayer(id):roadLayer(id);
